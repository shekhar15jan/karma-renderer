...
import type { VisualComponent, ThemeId, LayoutType } from "../core/types";
import type { Theme } from "../theme/themes";
import { paletteFor } from "../theme/themes";
import { iconSvg } from "../icons/icons";
import { contentColumns } from "../layout/layout";
...
function iconMarkup(name: string | undefined, size: number, color: string): string {
  return `<span style="color:${color};display:inline-flex;width:${size}px;height:${size}px;">${iconSvg(name, size)}</span>`;
}

function paletteStyle(theme: Theme, index: number, fill?: string, line?: string): string {
  const p = paletteFor(theme, index);
...
const REFERENCE_TONES: Record<string, { main: string; light: string; border: string }> = {
...
function toneOf(c: VisualComponent, index: number, theme: Theme, fallbackMain = "blue"): { main: string; light: string; border: string; pillBg: string } {
  const explicit = c.line;
  if (explicit) return { main: explicit, light: c.fill ?? "#ffffff", border: explicit, pillBg: explicit };
  const tone = (c.data?.tone as string | undefined) ?? fallbackMain;
  const t = REFERENCE_TONES[tone] ?? REFERENCE_TONES[fallbackMain];
  return { main: t.main, light: c.fill ?? t.light, border: c.line ?? t.border, pillBg: paletteFor(theme, index).line };
}

// ---------------------------------------------------------------- cards

function cardComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  ...
  const color = c.fill ?? paletteFor(theme, index).line;
      ...
      case "comparison": {
        ...
        return `<div class="comparison-head">${esc(label)}</div><table class="comparison-table"><tbody>` +
          cols
            .map(
              (col) =>
                `<tr><td class="cmp-key">${esc(col.header ?? "")}</td>` +
...
function containerComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  const theme = ctx.theme;
  const label = c.label ?? "";
  const items = (c.items ?? []).map((it) => (typeof it === "string" ? { label: it } : it));
  const isGlass = c.type === "glass-container";  const isGradient = c.type === "gradient-container";
...
function bannerComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  const theme = ctx.theme;
  const label = c.label ?? "";
...
function pillCardComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  const theme = ctx.theme;
  const label = c.label ?? "";
...
function pillarComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  const theme = ctx.theme;
  const label = c.label ?? "";
  const items = (c.items ?? []).map((it) => (typeof it === "string" ? { label: it } : it));
  const tone = toneOf(c, index, theme, "blue");
...
function timelineComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  const theme = ctx.theme;
  const items = (c.items ?? []).map((it) => (typeof it === "string" ? { label: it } : it));
  const horizontal = c.type === "horizontal-timeline" || ctx.layout === "roadmap";
  const lineColor = paletteFor(theme, index).line;
  const nodes = items
    .map((it, i) => {
...
function chartSvg(c: VisualComponent, ctx: ContentContext, index: number): string {
  ...
  const labels: string[] = (c.items ?? []).map((it) => (typeof it === "string" ? it : it.label));
  ...
  if (kind === "pie" || kind === "donut") {
    ...
    if (kind === "gauge") {
      ...
      inner =
        ...
        `<path d="M${x0},${y0} A${r},${r} 0 ${pct > 50 ? 1 : 0} 1 ${xv},${yv}" fill="none" stroke="${paletteFor(theme, 0).line}" stroke-width="18" stroke-linecap="round"/>` +
      ...
      inner =
        ...
        `<rect x="${bx}" y="${by}" width="${bw * (pct / 100)}" height="28" rx="14" fill="${paletteFor(theme, 0).line}"/>` +
...
function tableComponent(c: VisualComponent, ctx: ContentContext, index: number): string {
  ...
  const headers = c.data?.headers as string[] | undefined;
...
export function codeComponent(code: { lang?: string; text: string }, ctx: ContentContext): string {
  const theme = ctx.theme;
...
export function instructionsComponent(instructions: string[], ctx: ContentContext): string {
  const theme = ctx.theme;
...
export function renderContentScene(components: VisualComponent[], ctx: ContentContext): string {
  const theme = ctx.theme;
  const cols = contentColumns(ctx.layout);
  const isTimelineLayout = ctx.layout === "timeline" || ctx.layout === "roadmap" || ctx.layout === "learning-path" || ctx.layout === "sprint";

  const sections: string[] = [];
  for (let i = 0; i < components.length; i++) {
    const c = components[i];
    const t: string = c.type ?? "card";
    if (t === "code" && c.data?.text) {
      sections.push(codeComponent({ lang: c.data?.lang as string, text: String(c.data.text) }, ctx));
  ...
  return (
    `<div class="content-scene ${isTimelineLayout ? "content-timeline" : ""}">` +
