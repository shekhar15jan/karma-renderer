...
exports.markerDefsFor = markerDefsFor;
...
function markerDefsFor(key, color, kind) {
    const id = `arrow-${key}`;
    return `<defs>${markerDefs(id, color, kind)}</defs>`;
}
function markerDefs(id, color, kind) {
    const size = 12;
    const hw = size / 2;
...
function renderArrow(input, markerIdBase, theme) {
