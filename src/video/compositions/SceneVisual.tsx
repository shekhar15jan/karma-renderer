...
export const SceneVisual: React.FC<SceneVisualProps> = ({ html, theme, durationFrames, fps, animation, entrance = true }) => {
  ...
  useLayoutEffect(() => {
    ...
    elements.forEach((el, i) => {
      ...
      if (anim.bullets) {
        const bullets = Array.from(el.querySelectorAll<HTMLElement>("[data-bullet]"));
        if (bullets.length) {
    ...
    for (const h of highlights) {
      const target = root.querySelector<HTMLElement>(`[data-eid="${h.id}"]`);
      if (!target) continue;
      if (t >= h.at) {
    ...
    // --- chart draw-in --------------------------------------------------------------
    if (anim.drawCharts) {
      const bars = Array.from(root.querySelectorAll<SVGRectElement>("[data-chart-bar]"));
      const line = root.querySelector<SVGPolylineElement>("[data-chart-line]");
      const area = root.querySelector<SVGPolygonElement>("[data-chart-area]");
  ...
  return (
    <AbsoluteFill style={{ background: theme.background, ...themeCssVars(theme) }}>
      ...
      <AbsoluteFill style={{ transform: `scale(${zoom})`, transformOrigin: "center center", overflow: "hidden" }}>
        <div
          ref={rootRef}
